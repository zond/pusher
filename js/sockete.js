...
(function () {
  ...
  // knows about the client and request type ('open', 'Message', 'Authorize', 'close')
...
(function () {
  ...
  Sockete.Client = function (url) {
    ...
    this.send = function (msg) {
      ...
      var request = new Sockete.Request(self, type, msg);
      self.__server.request(request, dispatch);
      return true;
    }
    ...
    function dispatch (response) {
      ...
      if (response.type.match(/(open|close|Message)/) === null) response.type = 'message';
      self['on'+response.type.toLowerCase()](response);
    }
...
(function () {
  ...
  Sockete.Server.prototype = {
    ...
    request: function (request, callback) {
      var response;
      var responder = this.findResponder(request);
      console.log('REQ', request.request_type);
      if (responder) {
        ...
        switch(request.request_type) {
          case 'open': // should let client open connection
            ...
            break;
          case 'Authorize': // should let client ack the request
            response = new Sockete.Response(request.client, 'Ack');
            break;
